...
実在の企業ではなく、架空の生命保険会社「Sample生命保険株式会社」における 個人保険（終身保険・定期保険・医療保険・がん保険 等）の新契約手続きWebという題材で、AIDDフェーズ（D0〜）に沿った成果物を順次配置していきます。

## 何が置いてあるか

成果物本体は [docs/](docs/) 配下、対応するテンプレートは [templates/](templates/) 配下、Claude による生成・更新を支援するスキル定義は [.claude/skills/](.claude/skills/) 配下に、それぞれフェーズ区分で収録しています。

| フェーズ | 成果物本体 | テンプレート | スキル |
|---|---|---|---|
| **D0** プロジェクト管理 | [docs/D0.project-management/](docs/D0.project-management/) | — | — |
| **D1** ビジネス要求 | [docs/D1.business-requirements/](docs/D1.business-requirements/) | [templates/D1.business-requirements/](templates/D1.business-requirements/) | [.claude/skills/d1-business-requirement-document/](.claude/skills/d1-business-requirement-document/) |
...
- **テンプレート**: [templates/](templates/) 配下に、章構成と書き方ヒント（HINTコメント）を埋め込んだ雛形があります。スキルとあわせて利用されます
...
├── templates/                     # 成果物テンプレート（章構成・書き方ヒント入り）
│   └── D1.business-requirements/
├── .claude/
│   └── skills/                    # Claude スキル定義（フェーズ別）
