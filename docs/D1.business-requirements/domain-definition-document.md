...
flowchart LR
    ...
    DESIGN -.->|商品仕様・保険料を参照| PROD[PROD: 商品仕様管理]
    DESIGN -.->|情報提供の証跡| SUIT
    UNDW -.->|引受基準を参照| PROD
    APPL -.->|顧客情報を連携| CUST
    DECL -.->|被保険者情報を連携| CUST
    APPL -.->|募集権限を参照| CHNL[CHNL: 募集チャネル管理]
    APPL -.->|申込の証跡| SUIT
    ...
    DESIGN -.->|募集人取扱資格を参照| CHNL
    CHNL -.->|募集責任主体を提供| SUIT
    DESIGN -.->|意向見直し| HEAR

    classDef crosscut fill:#f5f1ea,stroke:#b8a99a,color:#000
    class PROD,CUST,CHNL,SUIT,KYC,ASF,ESIGN,AUDIT crosscut
```

> **凡例(矢印の意味):**
>
> - **実線 `A --> B`**: 業務工程の **前後関係**。A の完了が B の業務上の契機となる **時系列の流れ** を表す(例: 意向把握 → 設計書作成)。
> - **破線 `A -.-> B`**: **参照関係(依存関係)**。A が B の機能・データ・統制を業務上 **参照/依存** することを表す。時系列の順序ではなく「どのドメインが何に依存するか」を示す。横断ドメイン(顧客情報・商品仕様・証跡管理 等)への参照、および業務工程間のフィードバック(`DESIGN -.->|意向見直し| HEAR` = 設計書提示後の意向見直し)を含む。
>
