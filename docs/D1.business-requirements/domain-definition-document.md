...
業務工程ドメイン同士の前後関係(実線矢印)と、業務工程ドメインから横断ドメインへの主な参照関係(破線矢印)を示します。詳細なシーケンスは下流の L1/L2 シーケンス図に降ろし、本書では「どのドメインがどのドメインを参照するか」のレベルに留めます。
...
flowchart LR
    ...
    HEAR -.-> CUST[CUST: 顧客情報管理]
    HEAR -.-> SUIT[SUIT: 募集コンプライアンス証跡管理]
    DESIGN -.-> PROD[PROD: 商品仕様管理]
    DESIGN -.-> SUIT
    UNDW -.-> PROD
    APPL -.-> CUST
    DECL -.-> CUST
    APPL -.-> CHNL[CHNL: 募集チャネル管理]
    APPL -.-> SUIT
    APPL -.-> KYC[KYC: 本人確認]
    APPL -.-> ASF[ASF: 反社チェック]
    APPL -.-> ESIGN[ESIGN: 電子署名]
    DECL -.-> ESIGN
    BOOK -.-> AUDIT[AUDIT: 統制・証跡管理]

    classDef crosscut fill:#f5f1ea,stroke:#b8a99a,color:#000
    class PROD,CUST,CHNL,SUIT,KYC,ASF,ESIGN,AUDIT crosscut
```

> 凡例: 実線矢印は業務工程上の前後関係、破線矢印は横断ドメインへの参照関係を表します。図の可読性確保のため、横断ドメインへの参照は代表的な関係のみ描画しており、実際にはほぼすべての業務工程ドメインが CUST(顧客情報管理)・AUDIT(統制・証跡管理)を参照します。
