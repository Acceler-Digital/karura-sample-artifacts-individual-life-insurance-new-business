# 統制・証跡管理(アクセス制御・監査ログ・電子帳簿保存)ドメイン要求仕様書

## 本書について

### 概要

本書は、Sample生命保険株式会社 個人保険新契約システムの「統制・証跡管理(アクセス制御・監査ログ・電子帳簿保存)」ドメインに関するドメイン要求を記載したドキュメントです。
...
* 内部統制・電子帳簿保存の業務所管(コンプライアンス部・内部監査部)
* 統制・証跡管理ドメイン担当の開発・QA
* PdM / PM
...
|---|---|---|
| PRD-SEC-1 | FISC基準準拠 | 本ドメインのアクセス制御・監査ログ・証跡保全の設計・運用の最上位準拠基準となる |
| PRD-SEC-2 | 認証方式 | RBAC が前提とする利用者認証(MFA含む)との連携に効く |
| PRD-SEC-5 | RBAC・最小権限 | 本ドメインの中核。役割×組織×商品×募集チャネルの RBAC・最小権限・特権アクセス別系統管理の責務を担う |
| PRD-SEC-6 | 監査ログ(ログ対象・改ざん不能性) | 本ドメインの中核。個人情報・契約データへのアクセス・操作・参照、およびログ自体への改変を改ざん不能に記録・追跡する責務を担う |
| PRD-SEC-7 | 監査ログ(保存期間) | 本ドメインの中核。監査ログ・契約関係証跡の10年間保持の責務を担う |
| PRD-SEC-8 | 第三者セキュリティ診断 | 診断時の証跡提供・指摘対応の業務窓口としての位置づけに効く |
| PRD-SEC-9 | インシデント対応(エスカレーション) | 不正アクセス・改ざん検知時のCSIRT・コンプライアンス部等へのエスカレーション起点となる |
| PRD-SEC-DATA-1 | 顧客情報 | アクセス制御・参照監査の対象データの機密区分(個人情報)を継承する |
| PRD-SEC-DATA-2 | 告知情報(健康状態・診査結果・職業 等) | 要配慮個人情報への厳格なアクセス制御・参照監査の対象として継承する |
| PRD-SEC-DATA-6 | 募集コンプライアンス証跡 | 改ざん不能保存・参照監査の統制対象として継承する |
| PRD-SEC-DATA-7 | 監査ログ・操作ログ | 本ドメインが直接管理する中核データ。改ざん不能保存・10年間保持を担う |
| PRD-REG-3 | 個人情報保護法 | 要配慮個人情報を含む個人情報への厳格なアクセス制御・利用目的制限の統制に効く |
| PRD-REG-5 | 電子帳簿保存法 | 本ドメインの中核。電子保存する申込・契約関係書類の真実性確保・可視性確保(検索要件等)の統制責務を担う |
| PRD-REG-6 | 金融庁監督指針 | 内部管理態勢・お客様本位の業務運営を支える統制・証跡の整備に効く |
...
|---|---|---|---|
| AUDIT-BOP-1 | 権限を超えたアクセスの試行・検知 | 最小権限を超える参照・操作の試行 | アクセスを拒否し、試行を改ざん不能に記録する。要配慮個人情報への不正アクセス疑いはCSIRT・コンプライアンス部へエスカレーションする(PRD-SEC-9 に整合) |
| AUDIT-BOP-2 | 監査ログ自体への改変試行 | ログの改ざん・削除の試行 | 改変試行を別系統で全件追跡し、内部監査部・CSIRTへエスカレーションする(AUDIT-BR-5・PRD-SEC-9 に整合) |
| AUDIT-BOP-3 | 退職者・異動者の権限残存 | 人事異動・退職に伴う剥奪漏れ | 定期棚卸・人事連携で検知し、即時に権限を失効させる。残存期間中のアクセス有無を監査ログで遡及確認する(AUDIT-BR-10 に整合) |
| AUDIT-BOP-4 | 電子帳簿の真実性・可視性要件の不充足 | 真実性措置の付与失敗・検索要件の不備 | 当該書類を正式保存扱いとせず、真実性・可視性措置を再充足する。是正までの経緯を証跡として残す(AUDIT-BR-7・AUDIT-BR-8・PRD-REG-5 に整合) |
| AUDIT-BOP-5 | 保存期間内の証跡消失・破損の疑い | ストレージ障害・運用ミス 等 | 消失・破損範囲を特定し、保全手段で復元を図る。復元不能時はコンプライアンス部・内部監査部へエスカレーションし、影響と再発防止を記録する(AUDIT-BR-6 に整合) |
| AUDIT-BOP-6 | 緊急時の例外的アクセス(ブレイクグラス) | 障害対応・インシデント対応での特権行使 | 事前承認または事後速やかな承認を要件とし、行使内容を全件記録のうえ事後レビューに付す(AUDIT-BR-3 に整合)【要確認: 緊急時例外アクセスの承認方式(事前/事後)を業務所管で確定要】 |
| AUDIT-BOP-7 | 第三者セキュリティ診断での重大指摘 | リリース前診断での統制不備指摘 | 指摘事項を業務所管で受領し、是正完了をリリース判定の必須条件とする(PRD-SEC-8 に整合) |

## 他ドメインとの連携

本ドメインは横断ドメインであり、ほぼすべての業務工程ドメイン・横断ドメインから参照されます(ドメイン定義書の注記に整合)。代表的な連携を以下に示します。

| 方向 | 相手ドメイン | 連携内容 | 契機 |
|---|---|---|---|
| 入力 | 全業務工程ドメイン(HEAR・DESIGN・APPL・DECL・UNDW・PREM・BOOK・PLCY) | 個人情報・契約データへのアクセス・操作・参照イベントを受け取り改ざん不能に記録する | 各ドメインでのデータアクセス・操作のつど |
| 入力 | BOOK(契約成立) | 計上に伴う契約関係書類を電子帳簿保存の統制対象として受け取る | 契約成立(計上)時 |
| 入力 | KYC・ASF・ESIGN・SUIT | 本人確認・反社チェック・電子署名・募集コンプライアンスの証跡を改ざん不能保存・統制対象として受け取る | 各証跡の確定時 |
| 出力 | 全直接利用ドメイン | RBAC に基づくアクセス可否の判定結果を返す | データアクセス・操作の要求時 |
| 出力 | コンプライアンス部・内部監査部・CSIRT(統制・監督アクター) | 改ざんされていない監査証跡・アクセス記録を説明可能な形で提供する | 内部監査・第三者診断・インシデント対応の求めに応じて |
