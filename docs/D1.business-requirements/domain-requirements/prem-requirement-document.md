...
本書は、Sample生命保険株式会社 個人保険新契約システムの「第一回保険料収納」ドメインに関するドメイン要求を記載したドキュメントです。
...
* 新契約事務部門・収納業務のドメインエキスパート
* 第一回保険料収納ドメイン担当の開発・QA
* PdM / PM
...
|---|---|---|
| PRD-FR-1 | 業務通知 | 収納成立・収納不能・再依頼を節目イベントとして募集人・新契約事務担当者・申込人へ通知する局面に効く |
| PRD-NFR-1 | オンライン操作のレスポンスタイム | 収納手段選択・収納状況確認操作のレスポンス、決済サービス連携を伴う操作の応答時間に適用 |
| PRD-NFR-2 | スループット | 月末月初に集中する第一回保険料収納処理のピーク処理能力に適用 |
| PRD-NFR-3 | 可用性 | 収納処理の業務時間帯稼働率に適用。収納停滞は責任開始日・リードタイムKPIに直結 |
| PRD-NFR-4 | 障害時の縮退運用方針 | 外部決済サービス障害時の収納業務継続シナリオに適用 |
| PRD-NFR-6 | 監視・障害検知 | 収納成立検知の遅延・キュー滞留・外部連携応答の監視に直結 |
| PRD-NFR-8 | 既存システム・外部サービスの更改耐性 | 既存収納システム・外部決済サービスの更改・差し替えに対する改修範囲最小化に適用 |
| PRD-NFR-9 | 外部連携・非同期処理のエラー検知・リトライ・冪等性 | 収納成立検知・決済連携の冪等性・二重収納防止・リトライポリシーに直結 |
| PRD-SEC-2 | 認証方式 | 収納手段(口座・カード情報)を扱う操作の認証に適用 |
| PRD-SEC-3 | 通信時暗号化 | 決済情報の外部決済サービス連携時の暗号化に適用 |
| PRD-SEC-4 | 保存時暗号化 | 口座情報・カード情報等の決済関連個人情報の保存時暗号化に適用 |
| PRD-SEC-5 | RBAC・最小権限 | 収納情報・決済情報へのアクセス権限制御に適用 |
| PRD-SEC-6 | 監査ログ・改ざん不能性 | 収納成立・収納手段変更・申込撤回操作の改ざん不能記録に適用 |
| PRD-SEC-DATA-3 | 申込・契約情報(払込方法 等) | 払込方法・収納手段の確定情報を扱う。役割×組織での権限制御、募集人は自身の関与申込のみ |
| PRD-REG-3 | 個人情報保護法 | 口座・カード等の決済関連個人情報の取得・利用・保管に適用 |
| PRD-REG-6 | 金融庁監督指針 | 収納に関する内部管理態勢・お客様本位の業務運営原則を収納プロセスに担保 |
...
|---|---|---|---|
| PREM-BOP-1 | 口座振替の引落不能 | 残高不足・口座相違・振替不能 | PREM-BR-6 に従い再振替を試行。規定回数を超えたら代替手段提示(PREM-BR-7)へ移行 |
| PREM-BOP-2 | クレジットカードのオーソリ拒否 | 与信不足・カード無効・限度超過 | 申込人へ別カード/別手段への変更を提示し再試行。期限内に成立しなければ撤回対象 |
| PREM-BOP-3 | 振込・コンビニ払込の入金未確認 | 期限内に入金されない/消込できない | 入金督促・再案内を行い、期限超過時は収納未了撤回(PREM-BR-8) |
| PREM-BOP-4 | 外部決済サービスの障害・不達 | 決済サービスのダウン・タイムアウト・応答不達 | PRD-NFR-4 の縮退運用方針に従い収納業務を継続。復旧後にリトライし PREM-BR-10 で二重収納を防止 |
| PREM-BOP-5 | 収納成立検知の遅延 | 非同期検知の遅延・連携滞留 | PRD-NFR-6 の監視で検知し、滞留解消まで滞留管理。責任開始日確定はPREM-BR-4 に従い検知確定後に実施 |
| PREM-BOP-6 | 過収納・二重収納の疑い | 再依頼・連携再送と実収納が重複 | 冪等に収納成立を確定(PREM-BR-10)。二重収納が判明した場合は返金・取消を業務手順で実施 |
| PREM-BOP-7 | 引受査定の謝絶・申込撤回後に収納成立 | 査定結果確定前に収納が先行成立し、その後謝絶/撤回 | 契約不成立として収納金を返金・取消する業務手順を適用。BOOK(契約成立)へ不成立を伝達 |
| PREM-BOP-8 | 収納金額の事後変更 | 特別条件付与・プラン変更で確定額が変動 | 確定額(PREM-BR-2)に基づき差額の追加収納または返金を行い、整合した額で契約成立条件を充足させる |

## 他ドメインとの連携

| 方向 | 相手ドメイン | 連携内容 | 契機 |
|---|---|---|---|
| 入力 | APPL(申込受付) | 申込確定情報・払込方法・収納依頼を受け取る | 申込意思確定時 |
| 入力 | UNDW(引受査定) | 引受可決可否・特別条件(収納金額確定に必要)を参照する | 査定確定後の収納金額確定時 |
| 出力 | BOOK(契約成立) | 収納成立可否・責任開始日・収納未了撤回 を契約成立条件として引き渡す | 収納成立確定時/収納未了撤回時 |
| 出力 | CUST(顧客情報管理) | 申込人の決済関連個人情報の参照(横断参照) | 収納手段確定・収納依頼時 |
| 出力 | AUDIT(統制・証跡管理) | 収納成立・収納手段変更・撤回操作の改ざん不能証跡を保全依頼 | 収納成立・収納手段変更・撤回時 |
...
* 収納金額の正確性: 特別条件・プラン内容を反映した確定額で収納され、事後変動時も整合した額に収束すること(PREM-BR-2・PREM-BOP-8)
