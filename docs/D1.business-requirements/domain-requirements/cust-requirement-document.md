...
本書は、Sample生命保険株式会社 個人保険新契約システムの「顧客情報管理」ドメインに関するドメイン要求を記載したドキュメントです。
...
* 顧客情報管理・個人情報保護(個人情報保護管理者)のドメインエキスパート
* 顧客情報管理ドメイン担当の開発・QA
* PdM / PM
...
|---|---|---|
| PRD-FR-4 | 同意取得・同意管理 | 個人情報の取得・利用目的の明示と本人同意の取得・撤回・更新を扱う。本ドメインが同意情報の集約・管理の主担当となる |
| PRD-NFR-8 | 既存システム・外部サービスの更改耐性 | 契約成立後の既存契約管理システムへの顧客情報引き渡しを、契約管理システム更改に対し改修範囲を最小化する形で行う |
| PRD-SEC-2 | 認証方式 | 要配慮個人情報を含む顧客情報を扱う操作には MFA を必須とする前提に整合する |
| PRD-SEC-4 | 保存時暗号化 | 顧客情報・要配慮個人情報は保存時暗号化を必須とする |
| PRD-SEC-5 | RBAC・最小権限 | 顧客情報・要配慮個人情報へのアクセスは役割×組織×目的に基づき職務上必要な担当者に限定する |
| PRD-SEC-6 | ログ対象・改ざん不能性 | 顧客情報・要配慮個人情報への参照・操作を改ざん不能な形で記録する |
| PRD-SEC-9 | インシデント対応 | 顧客情報・要配慮個人情報の漏洩検知時のエスカレーション対象データを保有する |
| PRD-SEC-DATA-1 | データアクセス要件(顧客情報) | 取扱区分=個人情報。役割×組織×目的の最小権限制御、参照は監査ログ対象。本ドメインが集約・管理する中核データに直接対応 |
| PRD-SEC-DATA-2 | データアクセス要件(告知情報) | 取扱区分=要配慮個人情報。本ドメインは健康情報そのものの判定責務は持たないが、要配慮個人情報を含む顧客情報の取得目的・同意の管理に関与する |
| PRD-REG-2 | 保険法 | 被保険者・受取人の権利保護(受取人指定情報の正確な保持)を業務プロセスに組み込む |
| PRD-REG-3 | 個人情報保護法 | 個人情報の取得・利用・保管・削除を法令準拠で実施。要配慮個人情報の取得時本人同意・厳格なアクセス制御・利用目的特定を必須とする。本ドメインの中核法令要求 |
...
|---|---|---|---|
| CUST-BOP-1 | 名寄せ判定の不確実 | 氏名・生年月日等が近似するが同一人物と断定できない | 確証が得られない場合は別人物として扱い、誤統合を業務上回避する。要確認案件として人手確認に回す運用とする |
| CUST-BOP-2 | 同意の撤回 | 手続き途中で本人が個人情報利用への同意を撤回 | 撤回された目的での利用を停止。当該同意が手続き継続の前提である場合は、手続き継続不可として申込受付・告知受付側へ連絡し業務上の取り扱いを判定する |
| CUST-BOP-3 | 要配慮個人情報の取得同意未取得 | 健康情報を含む情報の取得時に必要な本人同意が未取得 | 同意未取得のまま要配慮個人情報を保持・利用しない。同意取得を業務手続き上の前提として差し戻す |
| CUST-BOP-4 | 顧客情報の不整合 | 同一人物に矛盾する属性(生年月日相違 等)が判明 | 最新の確証ある情報を確定情報とし、不整合を更新履歴として保全。確定できない場合は要確認として人手確認に回す |
| CUST-BOP-5 | 契約管理引き渡しの不達 | 契約成立後の契約管理システムへの引き渡しが不達 | 引き渡し対象は確定状態で保全し、回復可能な形で再連携する。引き渡し未了を業務上の未完了として追跡する(連携の信頼性方針は PRD-NFR-9 に従う) |
| CUST-BOP-6 | 個人情報の開示・訂正・削除請求 | 本人から保有個人情報の開示・訂正・利用停止の請求 | 法令に基づき請求に対応する。手続き進行中案件への影響を業務上判定し、関係ドメインへ連絡する |

## 他ドメインとの連携

| 方向 | 相手ドメイン | 連携内容 | 契機 |
|---|---|---|---|
| 入力 | HEAR(意向把握) | 意向把握で取得した顧客の家族構成・既契約状況 等を顧客情報として受け取る | 意向把握の実施時 |
| 入力 | APPL(申込受付) | 申込人・被保険者・受取人の属性・関係性・払込関連の本人情報を受け取り集約する | 申込受付での情報登録時 |
| 入力 | DECL(告知受付) | 要配慮個人情報を含む被保険者情報の取得目的・同意の管理に関与する | 告知受付の実施時 |
| 出力 | (既存契約管理システム) | 契約成立時に確定した顧客属性・関係性を引き渡す | 契約成立(引受可決+第一回保険料収納成立) |

> 補足: ドメイン定義書「ドメイン間の主な連携」では HEAR・APPL・DECL から CUST への参照(破線矢印)が明示され、ほぼすべての業務工程ドメインが CUST を参照すると注記されています。本ドメインは横断ドメインとして多数の業務工程ドメインから参照を受けます。
...
* 要配慮個人情報の厳格取扱い: 取得同意未取得の要配慮個人情報を保持・利用しないこと(CUST-BR-6 / CUST-BOP-3)
* 名寄せの安全性: 名寄せ不確実時に誤統合を回避し別人物扱いとする運用が確認されていること(CUST-BR-3 / CUST-BOP-1)
* 引き渡しの完全性: 契約成立時の契約管理システムへの顧客情報引き渡し対象が確定状態で保全され、不達時に回復可能であること(CUST-BR-8 / CUST-BOP-5)
