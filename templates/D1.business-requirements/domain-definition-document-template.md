...
【記述方針】
- 本プロダクトを構成する **ドメイン** (業務領域)を網羅する。
  これはプロダクトが扱う業務領域そのもの(問題空間)の整理であり、
  DDDにおける実装の境界 (Bounded Context) とは区別する。Bounded Context への
  マッピングは下流(D2 以降)で確定する。
- BRD のスコープに含まれる業務工程(設計・申込・査定 等)に加えて、
  業務工程をまたいで現れる横断ドメイン(顧客情報・商品マスタ・本人確認 等)も
  本書で取り扱う範囲とする。
- 各ドメインは1行に収める。詳細仕様(機能・画面・データ等)は
  プロダクト要求仕様書(PRD)・ドメイン要求仕様書群の下流に降ろす。
- 機能名・画面名・テーブル名・サービス名は混入させない(業務観点で書く)。
- 上流のビジネス要件定義書(BRD)のスコープ・対象業務と整合させる。
